...
The payment examples were subsequently improved at the user's request. The check and envelope now appear side by side at equal visual heights and aligned top edges on desktop, and stack below 901px. The envelope's transparent margins are clipped in CSS to align its visible edge with the check; original image files remain intact. Payee/memo and return/recipient addresses are positioned on the images using responsive HTML labels, with readable text copies underneath. `scripts/payment-diagrams.mjs` adds the diagrams at build time from the editable payment details, including after re-import. All four field values and image/label fit were checked at 1440, 768, 390, and 320px. Payment amounts and instructions were preserved.
